...
- **[AgentScrape](https://github.com/hshintelligence/agent-scrape)** - Pay-per-call web scraping for AI agents via the x402 protocol on Base USDC. Six tools (scrape, extract, screenshot, metadata, session, workflow) with no signup or API keys. REMOTE MCP at [https://agent-scrape.healingsunhaven.workers.dev/mcp](https://agent-scrape.healingsunhaven.workers.dev/mcp).
